...
static REGEX_EXPRESSION: &'static str = r"(\d+)-(\d+) ([a-z]): ([a-z]+)";
...
fn check_line_two(line: &String) -> bool {
    let (lower, upper, pivot, password) = parse_line(line);
    //dbg!(lower, upper, pivot, password);
    let pchars: Vec<char> = password.chars().collect();
    let pchars_len: u32 = pchars.len() as u32;
...
fn parse_line(line: &String) -> (u32, u32, char, &str) {
